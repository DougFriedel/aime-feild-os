...
const API = {
  projects:  { list:()=>supa("/projects?select=*&order=created_at.desc"), create:(d)=>supa("/projects",{method:"POST",body:d,prefer:"return=representation"}), update:(id,d)=>supa(`/projects?id=eq.${id}`,{method:"PATCH",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/projects?id=eq.${id}`,{method:"DELETE"}) },
  reports:   { forProject:(pid)=>supa(`/daily_reports?project_id=eq.${pid}&order=date.desc`), all:()=>supa("/daily_reports?select=*,projects(id,name)&order=date.desc&limit=300"), pending:()=>supa("/daily_reports?status=eq.submitted&select=*,projects(id,name)&order=created_at.desc"), create:(d)=>supa("/daily_reports",{method:"POST",body:d,prefer:"return=representation"}), update:(id,d)=>supa(`/daily_reports?id=eq.${id}`,{method:"PATCH",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/daily_reports?id=eq.${id}`,{method:"DELETE"}) },
  safety:    { forProject:(pid)=>supa(`/safety_logs?project_id=eq.${pid}&order=created_at.desc`), create:(d)=>supa("/safety_logs",{method:"POST",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/safety_logs?id=eq.${id}`,{method:"DELETE"}) },
  photos:    { forProject:(pid)=>supa(`/project_photos?project_id=eq.${pid}&order=created_at.desc`), create:(d)=>supa("/project_photos",{method:"POST",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/project_photos?id=eq.${id}`,{method:"DELETE"}) },
  timeCards: { forProject:(pid)=>supa(`/time_cards?project_id=eq.${pid}&order=date.desc,created_at.desc`), create:(d)=>supa("/time_cards",{method:"POST",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/time_cards?id=eq.${id}`,{method:"DELETE"}) },
  weather:   { forProject:(pid)=>supa(`/weather_logs?project_id=eq.${pid}&order=date.desc&limit=14`), upsert:(d)=>supa("/weather_logs",{method:"POST",body:d,prefer:"return=representation,resolution=merge-duplicates"}), remove:(id)=>supa(`/weather_logs?id=eq.${id}`,{method:"DELETE"}) },
  equipment: { forProject:(pid)=>supa(`/equipment_on_site?project_id=eq.${pid}&order=date.desc,created_at.desc`), create:(d)=>supa("/equipment_on_site",{method:"POST",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/equipment_on_site?id=eq.${id}`,{method:"DELETE"}) },
  subs:      { forProject:(pid)=>supa(`/subcontractors?project_id=eq.${pid}&order=date.desc,created_at.desc`), create:(d)=>supa("/subcontractors",{method:"POST",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/subcontractors?id=eq.${id}`,{method:"DELETE"}) },
  qc:        { forProject:(pid)=>supa(`/qc_forms?project_id=eq.${pid}&order=created_at.desc`), create:(d)=>supa("/qc_forms",{method:"POST",body:d,prefer:"return=representation"}), update:(id,d)=>supa(`/qc_forms?id=eq.${id}`,{method:"PATCH",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/qc_forms?id=eq.${id}`,{method:"DELETE"}) },
  docs:      { forProject:(pid)=>supa(`/documents?project_id=eq.${pid}&order=created_at.desc`), create:(d)=>supa("/documents",{method:"POST",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/documents?id=eq.${id}`,{method:"DELETE"}) },
  milestones:{ forProject:(pid)=>supa(`/milestones?project_id=eq.${pid}&order=sort_order.asc,target_date.asc`), create:(d)=>supa("/milestones",{method:"POST",body:d,prefer:"return=representation"}), update:(id,d)=>supa(`/milestones?id=eq.${id}`,{method:"PATCH",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/milestones?id=eq.${id}`,{method:"DELETE"}) },
  crew:      { list:()=>supa("/crew_members?order=name.asc"), create:(d)=>supa("/crew_members",{method:"POST",body:d,prefer:"return=representation"}), update:(id,d)=>supa(`/crew_members?id=eq.${id}`,{method:"PATCH",body:d,prefer:"return=representation"}), remove:(id)=>supa(`/crew_members?id=eq.${id}`,{method:"DELETE"}) },
  notifications:{ list:()=>supa("/notifications?order=created_at.desc&limit=50"), unread:()=>supa("/notifications?read=eq.false&order=created_at.desc"), markRead:(id)=>supa(`/notifications?id=eq.${id}`,{method:"PATCH",body:{read:true}}), markAllRead:()=>supa("/notifications?read=eq.false",{method:"PATCH",body:{read:true}}), create:(d)=>supa("/notifications",{method:"POST",body:d,prefer:"return=representation"}) },
  notifSettings:{ get:(name)=>supa(`/notification_settings?pm_name=eq.${encodeURIComponent(name)}&limit=1`), upsert:(d)=>supa("/notification_settings",{method:"POST",body:d,prefer:"return=representation,resolution=merge-duplicates"}) },
};

const T = {
  bg:"#09090B",surface:"#111113",card:"#18181B",border:"#27272A",
  orange:"#F97316",orangeLow:"#F9731614",orangeMid:"#F9731630",
  green:"#22C55E",greenLow:"#22C55E14",red:"#EF4444",redLow:"#EF444414",
  yellow:"#EAB308",yellowLow:"#EAB30814",blue:"#3B82F6",blueLow:"#3B82F614",
...
const POSITIONS = [
  {name:"Project Manager",rate:64.50},{name:"Foreman",rate:63.25},{name:"Technician",rate:60.75},
  {name:"Inspector",rate:53.75},{name:"Certified Welder",rate:60.75},{name:"Fitter",rate:58.50},
  {name:"Mechanic",rate:58.50},{name:"Operator",rate:58.50},{name:"Truck Driver",rate:58.50},
  {name:"Helper (Welder)",rate:57.25},{name:"Laborer",rate:51.00},{name:"Foreman (Elect)",rate:82.25},
  {name:"Electrician",rate:82.25},{name:"Helper (Elect)",rate:45.50},{name:"Per Diem",rate:190.00,flat:true},
];
const NAMES = ["Aden Walter","Alan Fairbrother","Alan Robinson","Artie Pugh","Ben Torres","Brandon Milano","Brandon Stach","Charles Dovel","Chris Utz","Christopher Dean","Chuck Dean","Clay Lau","Connor Kestner","Derrick Spriggs","Eric Bowens","Gary Czajkowski","Gavin Bowens","Gavin Kight","Howard Lau","James Gaskin","Jeff Lau","Jeff White","Jesse Harbarger","Jessica Vance","John Baier","John P. Cosner Jr.","Jordan Gorwell","Joseph Lau","Josh Gladhill","Julian Torres","Kevin Gabrish","Kevin Holmes","Kurt Batterton","Leo Velez","Logan Robinson","Mark Hamilton","Matt Kilby","Matt Pearlingi","Matthew Linton","Mike Gamble","Mike McBride","Mike Meridith","Mike Peterson","Mike Seiler","Nate Nemire","Owen Cross","Pat Gorman","Paul Howard","Rich Raborg","Robert Neslein","Roland Long","Shane Hower","Steve Kestner","Theodoro Torres","Thomas Britton","Tom Hatfield","Troy Strother","Tyrone Davis","Valentine Mbah","Walter Chicas-Luna","Will Wychulis","Wyatt Gill"].sort();
const EQUIP_LIST = [
  {section:"Trucks & Trailers"},
  {name:"Truck - 1 Ton",rate:21.50,unit:"Hours"},{name:"Truck - 3/4 Ton w/ Snow Plow",rate:350,unit:"Days"},{name:"Truck - 1/2 Ton",rate:18.50,unit:"Hours"},{name:"Truck - Boom (20-29 Ton)",rate:65,unit:"Hours"},{name:"Truck - Bucket",rate:45,unit:"Hours"},{name:"Truck - Dump Truck (3 Axle)",rate:35,unit:"Hours"},{name:"Truck - Haul Truck - No Trailer",rate:70,unit:"Hours"},{name:"Truck - Tru-Vac",rate:13500,unit:"Month"},{name:"Truck - Welding Rig",rate:35,unit:"Hours"},{name:"Trailer - Electrical - Colonial",rate:147,unit:"Month"},{name:"Trailer - Lowboy - 2 Axle",rate:28,unit:"Hours"},{name:"Trailer - Tag Along",rate:50,unit:"Days"},{name:"Trailer - Tool Trailer - 18-25ft",rate:175,unit:"Days"},{name:"Trailer - Tool Trailer - 26-40ft",rate:200,unit:"Days"},
  {section:"Earthmoving & ROW"},
  {name:"ATV - 4 Wheel",rate:125,unit:"Days"},{name:"Backhoe Loader - 80-105 HP",rate:62.45,unit:"Hours"},{name:"Excavator - Mini - 2-8K LB",rate:299,unit:"Days"},{name:"Excavator - Mini - 9K LB",rate:335,unit:"Days"},{name:"Excavator - Mini - 12-16K LB",rate:475,unit:"Days"},{name:"Excavator - Small - 21-29K LB",rate:565,unit:"Days"},{name:"Excavator - Small - 30-33K LB",rate:632,unit:"Days"},{name:"Excavator - Medium - 48-55K LB",rate:852,unit:"Days"},{name:"Excavator - Large - 80-89K LB",rate:1050,unit:"Days"},{name:"Excavator - Large - 90-119K LB",rate:1350,unit:"Days"},{name:"Skidsteer Loader - 70-80 HP",rate:440,unit:"Days"},{name:"Skidsteer Loader - 81-100 HP",rate:475,unit:"Days"},{name:"Tractor - 50 HP 4x4 w/ Bush Hog",rate:36.50,unit:"Hours"},{name:"Mower - Riding/Zero Turn",rate:175,unit:"Days"},
  {section:"Air, Compressors & Blast"},
  {name:"Air Compressor - 185 CFM",rate:195,unit:"Days"},{name:"Air Compressor - 375 CFM",rate:275,unit:"Days"},{name:"Air Impact Wrench - 1in",rate:50,unit:"Days"},{name:"Air Spade / Knife",rate:55,unit:"Days"},{name:"Blast Rig - 4 Bag Pot w/ 185 CFM AC",rate:55.50,unit:"Hours"},{name:"Blast Rig - 1 Pot w/ 375 CFM AC",rate:500,unit:"Days"},
  {section:"Testing & Misc. Tools"},
  {name:"Holiday Detector / Pipe Jeep",rate:72,unit:"Days"},{name:"Hydraulic Torque",rate:200,unit:"Days"},{name:"Hydro Test Pump",rate:60,unit:"Days"},{name:"Hydrotest - High Pressure",rate:3800,unit:"Days"},{name:"Jack Hammer",rate:72,unit:"Days"},{name:"LEL/Gas Monitor - 4 Gas",rate:50,unit:"Days"},{name:"Line Locator",rate:50,unit:"Days"},{name:"HEPA Vacuum",rate:100,unit:"Days"},{name:"Torque Wrench w/Sockets Hyd/Pneu",rate:195,unit:"Days"},{name:"Pipe Beveling Machine 16-22in",rate:100,unit:"Days"},
];
...
const QC_TEMPLATES = {
  "Pipeline Welding Inspection": ["Joint number recorded","Pipe heat numbers recorded","Fit-up inspection complete","Preheat verified","Root pass inspected","Fill passes inspected","Cap pass inspected","Visual inspection passed","Wall thickness verified","Bevel angle verified","Alignment within tolerance","NDE ordered if required"],
  "Hydrostatic Test Record": ["Test section isolated","Test medium approved (water)","Pressure gauge calibrated","Test pressure calculated and verified","Pressure chart recorder attached","Pressure achieved and held","Test duration met","No leaks observed","Pressure released safely","Test results recorded"],
  "Safety Pre-Task Analysis (PTA)": ["Hazards identified","Controls in place","PPE requirements reviewed","Emergency procedures reviewed","All crew briefed and signed","Work area inspected","Hot work permit obtained (if needed)","Excavation permit obtained (if needed)","Line clearance obtained (if needed)","First aid kit on site"],
  "Equipment Pre-Use Inspection": ["Fluid levels checked","Lights and signals operational","Tires/tracks inspected","Safety devices functional","No visible leaks","Controls operational","Fire extinguisher present","Documentation in cab","Pre-use inspection form completed","Operator licensed/certified"],
  "Coating Inspection": ["Surface prep method verified","Blast profile measured","Surface cleanliness verified","Holiday detection complete","Coating thickness measured","Repair areas re-inspected","Documentation complete","Inspector signature obtained"],
};
    ...
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",justifyContent:"center",padding:24,fontFamily:"inherit"}}>
      <div style={{textAlign:"center",marginBottom:40}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:12,marginBottom:10}}>
          ...
          <div style={{textAlign:"left"}}><div style={{fontSize:26,fontWeight:900,color:T.text,letterSpacing:"-1px",lineHeight:1.1}}>AIME Field OS</div><div style={{fontSize:11,color:T.muted,letterSpacing:"2px",textTransform:"uppercase"}}>Colonial Pipeline</div></div>
        </div>
        <div style={{fontSize:13,color:T.muted,marginTop:6}}>Daily reporting · Time cards · Safety · Documents · QC</div>
      </div>
      <div style={{...cardS,maxWidth:400,margin:"0 auto",width:"100%"}}>
        <div style={{marginBottom:14}}><label style={lbl}>Your Name</label><select value={name} onChange={e=>setName(e.target.value)} style={inp}><option value="">— Select your name —</option>{NAMES.map(n=><option key={n}>{n}</option>)}</select></div>
        <div style={{marginBottom:14}}><label style={lbl}>I am a…</label><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{["crew","pm"].map(r=>(<button key={r} onClick={()=>setRole(r)} style={{padding:"14px",borderRadius:12,border:`2px solid ${role===r?T.orange:T.border}`,background:role===r?T.orangeLow:T.surface,color:role===r?T.orange:T.sub,fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>{r==="crew"?"👷 Field Crew":"📋 Project Manager"}</button>))}</div></div>
        ...
        {role==="pm"&&<p style={{fontSize:11,color:T.muted,textAlign:"center",margin:"8px 0 0"}}>Default PIN: 1234</p>}
      </div>
    </div>
  );
}

/* ── PROJECTS HOME (JOB BOARD) ──────────────────────────────── */
function ProjectsHome({user,projects,loading,onSelect,onNew,onLogout,onDash,onCrew}){
  const [search,setSearch]=useState(""); const [filter,setFilter]=useState("active");
  const filtered=projects.filter(p=>{ const ms=filter==="all"?true:p.status===filter; const q=search.toLowerCase(); const ms2=!q||p.name?.toLowerCase().includes(q)||p.location?.toLowerCase().includes(q)||p.afe?.toLowerCase().includes(q)||p.client?.toLowerCase().includes(q)||p.work_order?.toLowerCase().includes(q); return ms&&ms2; });
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"16px 16px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><div style={{fontSize:17,fontWeight:900,color:T.text,letterSpacing:"-0.4px"}}>AIME Field OS</div><div style={{fontSize:11,color:T.muted}}>👋 {user.name} · {user.role==="pm"?"Project Manager":"Field Crew"}</div></div>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {user.role==="pm"&&<button onClick={onDash} style={{background:T.orangeLow,border:`1px solid ${T.orange}40`,borderRadius:10,padding:"8px 12px",color:T.orange,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>📊</button>}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16}}>
          {[{label:"Active Jobs",val:active.length,color:T.orange},{label:"Total Reports",val:totalReports,color:T.green},{label:"Total Billed",val:"$"+(totalBilled>=1000?(totalBilled/1000).toFixed(1)+"k":fmt(totalBilled)),color:T.blue}].map(s=>(<div key={s.label} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"10px 8px",textAlign:"center"}}><div style={{fontSize:17,fontWeight:900,color:s.color}}>{s.val}</div><div style={{fontSize:9,color:T.muted,textTransform:"uppercase",letterSpacing:"0.7px",marginTop:2}}>{s.label}</div></div>))}
        </div>
        <div style={{position:"relative",marginBottom:12}}>
          <span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",fontSize:15,pointerEvents:"none"}}>🔍</span>
          <input type="text" placeholder="Search jobs by name, location, AFE…" value={search} onChange={e=>setSearch(e.target.value)} style={{...inp,paddingLeft:40,borderRadius:12,fontSize:14}}/>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {[["active","Active"],["archived","Archived"],["all","All"]].map(([v,l])=>(<button key={v} onClick={()=>setFilter(v)} style={{padding:"9px 14px",borderRadius:"10px 10px 0 0",background:filter===v?T.bg:"transparent",border:filter===v?`1px solid ${T.border}`:"1px solid transparent",borderBottom:filter===v?`1px solid ${T.bg}`:"none",color:filter===v?T.text:T.muted,fontSize:13,fontWeight:filter===v?700:500,cursor:"pointer",fontFamily:"inherit",position:"relative",zIndex:filter===v?1:0,marginBottom:filter===v?-1:0}}>{l}{v==="active"&&active.length>0&&<span style={{marginLeft:5,background:T.orangeLow,color:T.orange,borderRadius:20,padding:"1px 6px",fontSize:10,fontWeight:800}}>{active.length}</span>}</button>))}
          {user.role==="pm"&&<button onClick={onNew} style={{marginLeft:"auto",background:T.orange,color:"#09090B",border:"none",borderRadius:10,padding:"9px 16px",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:2}}>+ New Job</button>}
        </div>
      </div>
      <div style={{padding:"14px 16px 100px"}}>
        {loading&&<Spinner/>}
        {!loading&&filtered.length===0&&(<div style={{textAlign:"center",padding:"60px 20px",color:T.muted}}><div style={{fontSize:48,marginBottom:12}}>🏗️</div><div style={{fontSize:17,fontWeight:700,color:T.sub,marginBottom:6}}>{search?`No jobs matching "${search}"`:filter==="archived"?"No archived jobs":"No active jobs"}</div><div style={{fontSize:13}}>{!search&&filter==="active"&&user.role==="pm"?"Tap + New Job to get started.":"Try a different search."}</div></div>)}
        {!loading&&filtered.map(p=><ProjectCard key={p.id} p={p} onSelect={onSelect}/>)}
      </div>
      {user.role==="pm"&&(<div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:T.surface,borderTop:`1px solid ${T.border}`,padding:"10px 16px",display:"flex",gap:8,zIndex:50}}><button style={{flex:1,background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"13px",color:T.text,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>🏗️ Jobs</button><button onClick={onDash} style={{flex:1,background:T.orange,border:"none",borderRadius:12,padding:"13px",color:"#09090B",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>📊 PM Dashboard</button></div>)}
    </div>
  );
}
    ...
    <div onClick={()=>onSelect(p)} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,marginBottom:10,cursor:"pointer",overflow:"hidden",opacity:isArchived?0.55:1}}>
      ...
      <div style={{padding:"14px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div style={{flex:1,minWidth:0,paddingRight:12}}><div style={{fontSize:17,fontWeight:900,color:T.text,letterSpacing:"-0.3px",lineHeight:1.2}}>{p.name}</div><div style={{fontSize:12,color:T.sub,marginTop:3}}>{[p.client,p.location].filter(Boolean).join(" · ")||"No details"}</div></div>
          <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:20,fontWeight:900,color:T.green,letterSpacing:"-0.5px"}}>${(p._billed||0)>=1000?((p._billed||0)/1000).toFixed(1)+"k":fmt(p._billed||0)}</div><div style={{fontSize:9,color:T.muted,textTransform:"uppercase",letterSpacing:"0.5px",marginTop:1}}>Total Billed</div></div>
        </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:10,borderTop:`1px solid ${T.border}`}}>
          <div style={{display:"flex",gap:16}}>
            <div><div style={{fontSize:16,fontWeight:800,color:T.orange}}>{p._reports||0}</div><div style={{fontSize:9,color:T.muted,textTransform:"uppercase",letterSpacing:"0.5px"}}>Reports</div></div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,background:T.orangeLow,border:`1px solid ${T.orange}40`,borderRadius:10,padding:"8px 14px"}}><span style={{fontSize:13,fontWeight:700,color:T.orange}}>Enter Job</span><span style={{fontSize:16,color:T.orange}}>→</span></div>
        </div>
      </div>
    </div>
  );
...
function DailyReportForm({user,project,onSave,onCancel}){
  ...
  async function submit(){
    setSaving(true);
    const saved=await onSave({...rpt,submitted_by:user.name,status:"submitted"});
    await notify("report_submitted","New Report Submitted",`${user.name} submitted a report for ${project.name} on ${fmtDate(rpt.date)}`,{project_id:project.id});
    setSaving(false);
  }
  ...
  return(
    <div ref={topRef} style={{background:T.bg,minHeight:"100vh",fontFamily:"inherit"}}>
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"14px 16px",position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",alignItems:"center"}}>{RSTEPS.map((s,i)=>(<div key={i} style={{display:"flex",alignItems:"center",flex:i<RSTEPS.length-1?1:undefined}}><div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}><div style={{width:26,height:26,borderRadius:"50%",background:i+1<step?T.green:i+1===step?T.orange:T.border,color:i+1<=step?"#09090B":T.muted,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800}}>{i+1<step?"✓":i+1}</div><div style={{fontSize:8,color:i+1===step?T.orange:T.muted,fontWeight:i+1===step?700:400,whiteSpace:"nowrap"}}>{s}</div></div>{i<RSTEPS.length-1&&<div style={{flex:1,height:2,background:i+1<step?T.green:T.border,margin:"0 3px",marginBottom:14}}/>}</div>))}</div>
      </div>
      <div style={{padding:"14px 16px 100px"}}>
        {step===1&&(<div><div style={{...cardS,marginBottom:14,borderLeft:`3px solid ${T.orange}`}}><div style={{fontSize:12,color:T.muted,marginBottom:3}}>Project</div><div style={{fontSize:15,fontWeight:700}}>{project.name}</div>{project.afe&&<div style={{fontSize:12,color:T.sub}}>AFE: {project.afe}{project.work_order?" · WO: "+project.work_order:""}</div>}</div><div style={{marginBottom:12}}><label style={lbl}>Report Date</label><input type="date" value={rpt.date} onChange={e=>setR("date",e.target.value)} style={inp}/></div><div style={{marginBottom:12}}><label style={lbl}>Report No.</label><input type="text" placeholder="Report #" value={rpt.report_no||""} onChange={e=>setR("report_no",e.target.value)} style={inp}/></div><div style={{marginBottom:12}}><label style={lbl}>Description of Work Done</label><textarea placeholder="Describe the work performed today…" value={rpt.description||""} onChange={e=>setR("description",e.target.value)} rows={4} style={{...inp,resize:"vertical",lineHeight:1.5}}/></div></div>)}
        {step===2&&(<div><div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div style={{fontSize:17,fontWeight:800}}>👷 Labor</div>{tot.labor>0&&<div style={{fontSize:16,fontWeight:800,color:T.green}}>${fmt(tot.labor)}</div>}</div>{rpt.labor.map((row,i)=><LaborCard key={row.id} row={row} onChange={r=>upd("labor",i,r)} onRemove={()=>del("labor",i)}/>)}<DashedAdd label="+ Add Worker" onClick={addLabor} color={T.orange}/></div>)}
        {step===3&&(<div><div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div style={{fontSize:17,fontWeight:800}}>🚜 Equipment</div>{tot.equip>0&&<div style={{fontSize:16,fontWeight:800,color:T.green}}>${fmt(tot.equip)}</div>}</div>{rpt.equipment.map((row,i)=><EquipCard key={row.id} row={row} onChange={r=>upd("equipment",i,r)} onRemove={()=>del("equipment",i)}/>)}<DashedAdd label="+ Add Equipment" onClick={addEquip} color={T.yellow}/></div>)}
        {step===4&&(<div><div style={{fontSize:17,fontWeight:800,marginBottom:12}}>📦 Materials & Misc.</div>{rpt.materials.map((row,i)=><MatCard key={row.id} row={row} onChange={r=>upd("materials",i,r)} onRemove={()=>del("materials",i)}/>)}<DashedAdd label="+ Add Material / Item" onClick={addMat} color={T.blue}/></div>)}
        {step===5&&(<div><div style={{fontSize:17,fontWeight:800,marginBottom:12}}>✅ Review & Submit</div><div style={{...cardS,marginBottom:12}}><div style={{fontSize:11,color:T.orange,fontWeight:700,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>Summary</div>{[["Project",project.name],["Date",fmtDate(rpt.date)],["Report No.",rpt.report_no||"—"],["Workers",rpt.labor.length],["Equipment",rpt.equipment.length+" items"],["Materials",rpt.materials.length+" items"]].map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:13,color:T.sub}}>{l}</span><span style={{fontSize:13,fontWeight:600}}>{v}</span></div>))}{[["Labor",tot.labor,T.green],["Equipment",tot.equip,T.green],["Materials",tot.mats,T.green]].map(([l,v,c])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:13,color:T.sub}}>{l}</span><span style={{fontSize:13,fontWeight:700,color:c}}>${fmt(v)}</span></div>))}<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:14}}><span style={{fontSize:16,fontWeight:800}}>Grand Total</span><span style={{fontSize:26,fontWeight:900,color:T.orange,letterSpacing:"-1px"}}>${fmt(tot.grand)}</span></div></div></div>)}
      </div>
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:T.bg+"EE",backdropFilter:"blur(12px)",borderTop:`1px solid ${T.border}`,padding:"12px 16px",display:"flex",gap:10}}>
        ...
        {step<5?<button onClick={()=>{setStep(s=>s+1);scroll();}} style={{...primBtn,flex:2,borderRadius:12}}>{step===4?"Review →":"Next →"}</button>:<button onClick={submit} style={{...primBtn,flex:2,borderRadius:12,opacity:saving?0.6:1}}>{saving?"Saving…":"💾 Save Report"}</button>}
      </div>
    </div>
  );
}
...
function ReportDetail({report,project,user,onBack,onDelete,onApprove,onFlag}){
  ...
  return(
    <div style={{background:T.bg,minHeight:"100vh",padding:16,fontFamily:"inherit"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}><div style={{fontSize:20,fontWeight:900,letterSpacing:"-0.5px"}}>{fmtDate(report.date)}</div><span style={pill(statusColor)}>{(report.status||"submitted").toUpperCase()}</span></div>
...
function CrewTab({projectId,user,onErr}){
  ...
  return(<div>
    ...
    {loading&&<Spinner/>}
    {!loading&&<>{todayEquip.length>0&&<div style={{fontSize:12,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>On Site Today</div>}
    {todayEquip.map(e=><div key={e.id} style={{...cardS,marginBottom:8,borderLeft:`3px solid ${T.yellow}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:14,fontWeight:700}}>{e.equipment_name}</div><div style={{fontSize:11,color:T.muted,marginTop:3}}>{fmtShort(e.date)} · Qty {e.quantity||1}{e.operator_name?" · "+e.operator_name:""}{e.hours_used?" · "+e.hours_used+"h":""}</div>{e.notes&&<div style={{fontSize:11,color:T.sub,marginTop:2}}>{e.notes}</div>}</div><button onClick={()=>remove(e.id)} style={{background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:16,padding:0,marginLeft:12}}>🗑</button></div>)}
    ...
    {prevEquip.map(e=><div key={e.id} style={{...cardS,marginBottom:8,borderLeft:`3px solid ${T.yellow}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:14,fontWeight:700}}>{e.equipment_name}</div><div style={{fontSize:11,color:T.muted,marginTop:3}}>{fmtShort(e.date)} · Qty {e.quantity||1}{e.operator_name?" · "+e.operator_name:""}</div></div><button onClick={()=>remove(e.id)} style={{background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:16,padding:0,marginLeft:12}}>🗑</button></div>)}
...
/* ── QC FORMS TAB ───────────────────────────────────────────── */
function QCTab({projectId,user,onErr}){
  ...
  const [template,setTemplate]=useState("Pipeline Welding Inspection");
  const [customName,setCustomName]=useState("");
  const [formDate,setFormDate]=useState(today());
  const [saving,setSaving]=useState(false);

  async function load(){setLoading(true);try{setForms(await API.qc.forProject(projectId)||[]);}catch(e){onErr(e.message);}setLoading(false);}
  useEffect(()=>{load();},[projectId]);
  ...
  async function createForm(){
    ...
    if(!name.trim())return;
    setSaving(true);
    const items=(QC_TEMPLATES[name]||[]).map(label=>({id:uid(),label,checked:false,notes:""}));
    try{
      const [created]=await API.qc.create({project_id:projectId,form_name:name,form_type:"checklist",date:formDate,items,status:"open",created_by:user.name});
      await load(); setMode("view"); setActiveForm(created);
    }catch(e){onErr(e.message);}
    setSaving(false);
  }

  async function updateItem(formId,items){
    try{
      const allChecked=items.every(i=>i.checked);
      const [updated]=await API.qc.update(formId,{items,status:allChecked?"completed":"open"});
      setActiveForm(updated);
      await load();
    }catch(e){onErr(e.message);}
  }

  async function signOff(formId){
    if(!window.confirm("Sign off and complete this form?"))return;
    try{
      const [updated]=await API.qc.update(formId,{status:"signed",signed_by:user.name,signed_at:new Date().toISOString()});
      setActiveForm(updated); await load();
    }catch(e){onErr(e.message);}
  }

  async function del(id){try{await API.qc.remove(id);await load();setMode("list");setActiveForm(null);}catch(e){onErr(e.message);}}

  const statusColor={open:T.yellow,completed:T.blue,signed:T.green};

  if(mode==="new") return(
    <div>
      <button onClick={()=>setMode("list")} style={{...ghostBtn,marginBottom:14}}>← Back</button>
      <div style={{fontSize:17,fontWeight:800,marginBottom:14}}>New QC Form</div>
      <div style={{marginBottom:12}}><label style={lbl}>Date</label><input type="date" value={formDate} onChange={e=>setFormDate(e.target.value)} style={inp}/></div>
      <div style={{marginBottom:12}}><label style={lbl}>Template</label>
        <select value={template} onChange={e=>setTemplate(e.target.value)} style={inp}>
          {Object.keys(QC_TEMPLATES).map(t=><option key={t} value={t}>{t}</option>)}
          <option value="custom">Custom Checklist…</option>
        </select>
      </div>
      {template==="custom"&&<div style={{marginBottom:12}}><label style={lbl}>Form Name</label><input type="text" placeholder="e.g. Valve Installation Inspection" value={customName} onChange={e=>setCustomName(e.target.value)} style={inp}/></div>}
      {template!=="custom"&&<div style={{...cardS,marginBottom:14,background:T.surface}}><div style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:8}}>Includes {(QC_TEMPLATES[template]||[]).length} items</div>{(QC_TEMPLATES[template]||[]).slice(0,5).map((item,i)=><div key={i} style={{fontSize:13,color:T.sub,padding:"4px 0",borderBottom:i<4?`1px solid ${T.border}`:"none"}}>☐ {item}</div>)}{(QC_TEMPLATES[template]||[]).length>5&&<div style={{fontSize:12,color:T.muted,marginTop:4}}>+ {(QC_TEMPLATES[template]||[]).length-5} more items…</div>}</div>}
      <button onClick={createForm} style={{...primBtn,borderRadius:14,opacity:saving?0.6:1}}>{saving?"Creating…":"Create Form"}</button>
    </div>
  );

  if(mode==="view"&&activeForm) return(
    <div>
      <button onClick={()=>{setMode("list");setActiveForm(null);}} style={{...ghostBtn,marginBottom:14}}>← QC Forms</button>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
        <div style={{fontSize:17,fontWeight:800,flex:1,paddingRight:10}}>{activeForm.form_name}</div>
        <span style={pill(statusColor[activeForm.status]||T.muted)}>{(activeForm.status||"open").toUpperCase()}</span>
      </div>
      <div style={{fontSize:12,color:T.muted,marginBottom:16}}>{fmtDate(activeForm.date)} · {activeForm.created_by}</div>
      {activeForm.signed_by&&<div style={{...cardS,marginBottom:14,background:T.greenLow,border:`1px solid ${T.green}40`}}><div style={{fontSize:13,color:T.green,fontWeight:700}}>✅ Signed off by {activeForm.signed_by}</div><div style={{fontSize:11,color:T.muted}}>{activeForm.signed_at?new Date(activeForm.signed_at).toLocaleString():""}</div></div>}
      <div style={{...cardS,marginBottom:14}}>
        {(activeForm.items||[]).map((item,i)=>{
          const items=[...(activeForm.items||[])];
          return(
            <div key={item.id} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"10px 0",borderBottom:i<activeForm.items.length-1?`1px solid ${T.border}`:"none"}}>
              <button onClick={()=>{items[i]={...item,checked:!item.checked};updateItem(activeForm.id,items);}} style={{width:24,height:24,borderRadius:6,border:`2px solid ${item.checked?T.green:T.border}`,background:item.checked?T.green:T.surface,color:"#09090B",cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,marginTop:1}}>{item.checked?"✓":""}</button>
              <div style={{flex:1}}><div style={{fontSize:14,color:item.checked?T.muted:T.text,textDecoration:item.checked?"line-through":"none",lineHeight:1.4}}>{item.label}</div></div>
            </div>
          );
        })}
      </div>
      <div style={{marginBottom:10}}><div style={{fontSize:12,color:T.muted,marginBottom:6}}>{(activeForm.items||[]).filter(i=>i.checked).length} / {(activeForm.items||[]).length} items complete</div><div style={{height:6,background:T.border,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",background:T.green,borderRadius:3,width:`${((activeForm.items||[]).filter(i=>i.checked).length/(activeForm.items||[]).length||0)*100}%`,transition:"width 0.3s"}}/></div></div>
      {activeForm.status!=="signed"&&(activeForm.items||[]).every(i=>i.checked)&&<button onClick={()=>signOff(activeForm.id)} style={{...primBtn,background:T.green,color:"#09090B",marginBottom:10,borderRadius:14}}>✅ Sign Off & Complete</button>}
      <button onClick={()=>del(activeForm.id)} style={dangerBtn}>🗑 Delete Form</button>
    </div>
  );

  return(
    <div>
      <button onClick={()=>setMode("new")} style={{...primBtn,marginBottom:14,borderRadius:14}}>📋 + New QC Form</button>
      {loading&&<Spinner/>}
      {!loading&&forms.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:T.muted}}><div style={{fontSize:32,marginBottom:8}}>📋</div><div>No QC forms yet. Tap above to create one.</div></div>}
      {!loading&&forms.map(form=>{
        const checked=(form.items||[]).filter(i=>i.checked).length;
        const total=(form.items||[]).length;
        return(<div key={form.id} onClick={()=>{setActiveForm(form);setMode("view");}} style={{...cardS,marginBottom:10,cursor:"pointer",borderLeft:`3px solid ${statusColor[form.status]||T.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div style={{flex:1}}><div style={{fontSize:15,fontWeight:700}}>{form.form_name}</div><div style={{fontSize:11,color:T.muted,marginTop:3}}>{fmtDate(form.date)} · {form.created_by}</div></div>
            <span style={pill(statusColor[form.status]||T.muted)}>{(form.status||"open").toUpperCase()}</span>
          </div>
          {total>0&&<div style={{marginTop:10}}><div style={{fontSize:11,color:T.muted,marginBottom:4}}>{checked}/{total} items</div><div style={{height:4,background:T.border,borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",background:form.status==="signed"?T.green:T.blue,width:`${(checked/total)*100}%`}}/></div></div>}
        </div>);
      })}
    </div>
  );
...
function DocsTab({projectId,user,onErr}){
  ...
  async function save(){
    ...
    try{await API.docs.create({...f,project_id:projectId,uploaded_by:user.name});await load();setShowForm(false);setF({name:"",doc_type:"Drawing",file_url:"",notes:""});}
  ...
  const grouped={};
  docs.forEach(d=>{const t=d.doc_type||"Other";if(!grouped[t])grouped[t]=[];grouped[t].push(d);});
  ...
  return(<div>
    ...
    {showForm&&<div style={{...cardS,marginBottom:14,borderLeft:`3px solid ${T.teal}`}}>
      <div style={{marginBottom:10}}><label style={lbl}>Document Name *</label><input type="text" placeholder="e.g. P&ID Drawing Rev 3" value={f.name} onChange={e=>set("name",e.target.value)} style={inp}/></div>
      <div style={{marginBottom:10}}><label style={lbl}>Document Type</label><select value={f.doc_type} onChange={e=>set("doc_type",e.target.value)} style={inp}>{docTypes.map(t=><option key={t}>{t}</option>)}</select></div>
      <div style={{marginBottom:10}}>
        <label style={lbl}>Document Link (Google Drive, Dropbox, SharePoint…)</label>
        <input type="url" placeholder="https://drive.google.com/…" value={f.file_url} onChange={e=>set("file_url",e.target.value)} style={inp}/>
        <div style={{fontSize:11,color:T.muted,marginTop:4}}>Paste a shareable link to the document. Tip: store docs in Google Drive and paste the share link here.</div>
      </div>
      <div style={{marginBottom:10}}><label style={lbl}>Notes / Revision</label><input type="text" placeholder="e.g. Rev 3 - Approved 2024-01" value={f.notes} onChange={e=>set("notes",e.target.value)} style={inp}/></div>
    ...
    {!loading&&Object.entries(grouped).map(([type,typeDocs])=>(
      <div key={type} style={{marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:8}}>{docIcons[type]||"📁"} {type} ({typeDocs.length})</div>
        {typeDocs.map(doc=>(
          <div key={doc.id} style={{...cardS,marginBottom:8,borderLeft:`3px solid ${T.teal}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                {doc.notes&&<div style={{fontSize:12,color:T.sub,marginBottom:4}}>{doc.notes}</div>}
                <div style={{fontSize:11,color:T.muted}}>{fmtDate(doc.created_at?.split("T")[0])} · {doc.uploaded_by}</div>
              </div>
              <div style={{display:"flex",gap:8,flexShrink:0,marginLeft:10}}>
                {doc.file_url&&<button onClick={()=>window.open(doc.file_url,"_blank")} style={{background:T.teal+"20",border:`1px solid ${T.teal}40`,borderRadius:8,padding:"6px 12px",color:T.teal,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Open →</button>}
                <button onClick={()=>remove(doc.id)} style={{background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:16,padding:0}}>🗑</button>
              </div>
            </div>
          </div>
        ))}
...
function ScheduleTab({projectId,user,onErr}){
  const [milestones,setMilestones]=useState([]); const [loading,setLoading]=useState(true);
  const [showForm,setShowForm]=useState(false); const [saving,setSaving]=useState(false);
  ...
  async function save(){
    ...
    try{await API.milestones.create({...f,project_id:projectId,sort_order:milestones.length});await load();setShowForm(false);setF({title:"",description:"",target_date:"",status:"pending"});}
    catch(e){onErr(e.message);}setSaving(false);
  }
  ...
  async function toggleStatus(m){
    ...
    try{const [u]=await API.milestones.update(m.id,{status:next[m.status],completed_date});setMilestones(ms=>ms.map(x=>x.id===m.id?u:x));}
    catch(e){onErr(e.message);}
  }
  ...
  return(<div>
    ...
    {showForm&&<div style={{...cardS,marginBottom:14,borderLeft:`3px solid ${T.blue}`}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        <div><label style={lbl}>Target Date</label><input type="date" value={f.target_date} onChange={e=>set("target_date",e.target.value)} style={inp}/></div>
        <div><label style={lbl}>Status</label><select value={f.status} onChange={e=>set("status",e.target.value)} style={inp}><option value="pending">Pending</option><option value="in_progress">In Progress</option><option value="completed">Completed</option><option value="delayed">Delayed</option></select></div>
      </div>
      <button onClick={save} style={{...primBtn,background:T.blue,borderRadius:12}}>{saving?"Saving…":"Save Milestone"}</button>
    </div>}
    {loading&&<Spinner/>}
    {!loading&&total>0&&<div style={{...cardS,marginBottom:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <div style={{fontSize:13,fontWeight:700}}>Progress</div>
        <div style={{fontSize:13,fontWeight:700,color:T.green}}>{completed}/{total} complete</div>
      </div>
      <div style={{height:8,background:T.border,borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",background:`linear-gradient(90deg,${T.orange},${T.green})`,borderRadius:4,width:`${(completed/total)*100}%`,transition:"width 0.4s"}}/></div>
    </div>}
    {!loading&&milestones.length===0&&!showForm&&<div style={{textAlign:"center",padding:"40px 0",color:T.muted}}><div style={{fontSize:32,marginBottom:8}}>📅</div><div>No milestones yet. Add project phases and target dates.</div></div>}
    {!loading&&milestones.map(m=>{
      ...
      return(<div key={m.id} style={{...cardS,marginBottom:10,borderLeft:`3px solid ${statusColor[m.status]||T.border}`,opacity:m.status==="completed"?0.7:1}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:15,fontWeight:700,textDecoration:m.status==="completed"?"line-through":"none",color:m.status==="completed"?T.muted:T.text}}>{m.title}</div>
            {m.description&&<div style={{fontSize:12,color:T.sub,marginTop:2}}>{m.description}</div>}
            <div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap"}}>
              <span style={pill(statusColor[m.status]||T.muted)}>{m.status.replace("_"," ").toUpperCase()}</span>
              {m.target_date&&<span style={pill(overdue?T.red:dueSoon?T.yellow:T.muted)}>{overdue?`${Math.abs(du)}d overdue`:dueSoon?`Due in ${du}d`:`Target: ${fmtDate(m.target_date)}`}</span>}
              {m.completed_date&&<span style={pill(T.green)}>Done: {fmtDate(m.completed_date)}</span>}
            </div>
          </div>
          <button onClick={()=>del(m.id)} style={{background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:16,padding:0,flexShrink:0}}>🗑</button>
        </div>
      </div>);
    })}
  </div>);
...
function PhotosTab({projectId,photos,onRefresh,onErr}){
  ...
  return(<div>
    ...
    <div style={{...cardS,marginBottom:14,borderStyle:"dashed",borderColor:T.orange+"44"}}>
      <div style={{marginBottom:10}}><label style={lbl}>Caption (optional)</label><input type="text" placeholder="What is this photo of?" value={caption} onChange={e=>setCaption(e.target.value)} style={inp}/></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      {photos.map(p=>(<div key={p.id} style={{position:"relative",borderRadius:12,overflow:"hidden",aspectRatio:"4/3",background:T.card}}><img src={p.src} alt={p.caption} onClick={()=>setLb(p.src)} style={{width:"100%",height:"100%",objectFit:"cover",cursor:"pointer",display:"block"}}/><button onClick={()=>del(p.id)} style={{position:"absolute",top:6,right:6,background:"rgba(0,0,0,0.75)",border:"none",color:"#fff",borderRadius:"50%",width:24,height:24,fontSize:12,cursor:"pointer"}}>×</button>{p.caption&&<div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.65)",color:"#fff",fontSize:11,padding:"4px 8px"}}>{p.caption}</div>}<div style={{position:"absolute",top:6,left:6,background:"rgba(0,0,0,0.6)",borderRadius:6,padding:"2px 6px",fontSize:9,color:"#fff"}}>{fmtShort(p.date)}</div></div>))}
    </div>
  </div>);
}
...
function WeatherTab({projectId,project,weather,onRefresh,onErr}){
  ...
  async function autoFetch(){if(!project.location){onErr("Add a location to this job first (Info tab).");return;}setFetching(true);setLiveWeather(null);try{setLiveWeather(await fetchWeather(project.location));}catch(e){onErr(e.message);}setFetching(false);}
...
function InfoTab({project,user,onEdit,onArchive}){
  return(<div>
    <div style={cardS}>{[["Client",project.client],["Location",project.location],["AFE No.",project.afe],["Work Order",project.work_order],["Start Date",fmtDate(project.start_date)],["Status",project.status],["Created By",project.created_by]].map(([l,v])=>v?(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"11px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:13,color:T.muted}}>{l}</span><span style={{fontSize:13,fontWeight:600}}>{v}</span></div>):null)}</div>
    ...
    {user.role==="pm"&&<div style={{marginTop:16,display:"flex",flexDirection:"column",gap:10}}><button onClick={onEdit} style={{...ghostBtn,width:"100%",textAlign:"center"}}>✏️ Edit Job</button><button onClick={onArchive} style={{...ghostBtn,width:"100%",textAlign:"center",color:T.muted}}>{project.status==="active"?"📦 Archive Job":"♻️ Restore Job"}</button></div>}
  </div>);
}
...
function CrewDirectoryScreen({onBack,user}){
  ...
  async function save(){
    ...
    try{
      ...
      await load();setMode("list");setActive(null);setF({...blankMember});
    }catch(e){setErr(e.message);}setSaving(false);
  }

  async function remove(id){if(!window.confirm("Remove this crew member?"))return;try{await API.crew.remove(id);await load();setMode("list");setActive(null);}catch(e){setErr(e.message);}}
  ...
  const inactive_members=members.filter(m=>!m.active);

  if(mode==="new"||mode==="edit") return(
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:"inherit"}}>
      <TopBar title={mode==="edit"?"Edit Crew Member":"Add Crew Member"} onBack={()=>{setMode("list");setActive(null);setF({...blankMember});}}/>
      <div style={{padding:"16px 16px 100px"}}>
        <ErrBanner msg={err} onDismiss={()=>setErr("")}/>
        <div style={{marginBottom:12}}><label style={lbl}>Full Name *</label><select value={f.name} onChange={e=>set("name",e.target.value)} style={inp}><option value="">— Select from list —</option>{NAMES.map(n=><option key={n}>{n}</option>)}</select></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <div><label style={lbl}>Cell Phone</label><input type="tel" placeholder="555-555-5555" value={f.phone} onChange={e=>set("phone",e.target.value)} style={inp}/></div>
          <div><label style={lbl}>Email</label><input type="email" placeholder="email@example.com" value={f.email} onChange={e=>set("email",e.target.value)} style={inp}/></div>
        </div>
        <div style={{...cardS,marginBottom:12}}>
          ...
          <div style={{marginBottom:10}}><label style={lbl}>Contact Name</label><input type="text" placeholder="Spouse, parent, etc." value={f.emergency_contact_name} onChange={e=>set("emergency_contact_name",e.target.value)} style={inp}/></div>
          <div><label style={lbl}>Contact Phone</label><input type="tel" placeholder="555-555-5555" value={f.emergency_contact_phone} onChange={e=>set("emergency_contact_phone",e.target.value)} style={inp}/></div>
        </div>
        <div style={{...cardS,marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <button onClick={addCert} style={{background:T.blueLow,border:`1px solid ${T.blue}40`,borderRadius:8,padding:"6px 12px",color:T.blue,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>+ Add</button>
          </div>
          {(f.certifications||[]).length===0&&<div style={{fontSize:13,color:T.muted,textAlign:"center",padding:"10px 0"}}>No certifications added.</div>}
          {(f.certifications||[]).map((cert,i)=>(
            <div key={cert.id} style={{borderTop:`1px solid ${T.border}`,paddingTop:10,marginTop:i>0?10:0}}>
              <div style={{marginBottom:8}}><label style={lbl}>Certification</label><select value={cert.name} onChange={e=>updateCert(i,"name",e.target.value)} style={inp}><option value="">— Select —</option>{CERT_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div><label style={lbl}>Cert Number</label><input type="text" placeholder="Optional" value={cert.cert_number} onChange={e=>updateCert(i,"cert_number",e.target.value)} style={inp}/></div>
                <div><label style={lbl}>Expiry Date</label><input type="date" value={cert.expiry} onChange={e=>updateCert(i,"expiry",e.target.value)} style={inp}/></div>
              </div>
              <button onClick={()=>removeCert(i)} style={{background:"none",border:"none",color:T.red,cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit"}}>Remove</button>
            </div>
          ))}
        </div>
        ...
        <button onClick={save} style={{...primBtn,opacity:f.name&&!saving?1:0.5}}>{saving?"Saving…":mode==="edit"?"Save Changes":"Add to Directory"}</button>
      </div>
    </div>
  );
    ...
    <div style={{background:T.bg,minHeight:"100vh",padding:16,fontFamily:"inherit"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
        <div>
          ...
          {active.classification&&<div style={{fontSize:14,color:T.sub,marginTop:2}}>{active.classification}</div>}
          <span style={{...pill(active.active?T.green:T.muted),marginTop:6,display:"inline-flex"}}>{active.active?"Active":"Inactive"}</span>
        </div>
        <button onClick={()=>{setF({...blankMember,...active,certifications:active.certifications||[]});setMode("edit");}} style={{background:T.orangeLow,border:`1px solid ${T.orange}40`,borderRadius:10,padding:"8px 14px",color:T.orange,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>✏️ Edit</button>
      </div>
      {(active.phone||active.email)&&<div style={{...cardS,marginBottom:12}}>
        <div style={{fontSize:12,fontWeight:700,color:T.blue,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>Contact</div>
        {active.phone&&<div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:13,color:T.muted}}>Cell</span><a href={`tel:${active.phone}`} style={{fontSize:13,fontWeight:600,color:T.blue,textDecoration:"none"}}>{active.phone}</a></div>}
        {active.email&&<div style={{display:"flex",justifyContent:"space-between",padding:"8px 0"}}><span style={{fontSize:13,color:T.muted}}>Email</span><a href={`mailto:${active.email}`} style={{fontSize:13,fontWeight:600,color:T.blue,textDecoration:"none"}}>{active.email}</a></div>}
      </div>}
      {(active.emergency_contact_name||active.emergency_contact_phone)&&<div style={{...cardS,marginBottom:12,borderLeft:`3px solid ${T.red}`}}>
        <div style={{fontSize:12,fontWeight:700,color:T.red,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>🆘 Emergency Contact</div>
        {active.emergency_contact_name&&<div style={{fontSize:14,fontWeight:700,marginBottom:4}}>{active.emergency_contact_name}</div>}
        {active.emergency_contact_phone&&<a href={`tel:${active.emergency_contact_phone}`} style={{fontSize:14,color:T.red,textDecoration:"none",fontWeight:700}}>📞 {active.emergency_contact_phone}</a>}
      </div>}
      {(active.certifications||[]).length>0&&<div style={{...cardS,marginBottom:12}}>
        <div style={{fontSize:12,fontWeight:700,color:T.blue,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>🎖️ Certifications</div>
        {(active.certifications||[]).map((cert,i)=>{
          ...
          return(<div key={cert.id||i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<active.certifications.length-1?`1px solid ${T.border}`:"none"}}>
            ...
            {cert.expiry&&<span style={pill(expired?T.red:expiring?T.yellow:T.green)}>{expired?"EXPIRED":expiring?`Exp ${exp}d`:fmtDate(cert.expiry)}</span>}
          </div>);
        })}
      </div>}
      ...
      {user.role==="pm"&&<button onClick={()=>remove(active.id)} style={{...dangerBtn,marginTop:8}}>Remove from Directory</button>}
    </div>
  );
    ...
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:"inherit"}}>
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"14px 16px",position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={()=>{setF({...blankMember});setMode("new");}} style={{background:T.orange,color:"#09090B",border:"none",borderRadius:10,padding:"8px 14px",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>+ Add</button>
        </div>
      </div>
      <div style={{padding:"14px 16px 80px"}}>
        ...
        {loading&&<Spinner/>}
        {!loading&&<>
          {active_members.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:T.muted}}><div style={{fontSize:36,marginBottom:8}}>👥</div><div>No crew members yet. Tap + Add to build your directory.</div></div>}
          {active_members.map(m=>{
            ...
            return(<div key={m.id} onClick={()=>{setActive(m);setMode("view");}} style={{...cardS,marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:44,height:44,borderRadius:12,background:T.orangeLow,border:`2px solid ${T.orange}40`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:800,color:T.orange,flexShrink:0}}>{m.name.split(" ").map(w=>w[0]).slice(0,2).join("")}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:15,fontWeight:700}}>{m.name}</div>
                <div style={{fontSize:12,color:T.sub}}>{m.classification||"No classification"}{m.phone?" · "+m.phone:""}</div>
          ...
          {inactive_members.length>0&&<>
            <div style={{fontSize:12,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",margin:"20px 0 10px"}}>Inactive</div>
            {inactive_members.map(m=>(<div key={m.id} onClick={()=>{setActive(m);setMode("view");}} style={{...cardS,marginBottom:8,cursor:"pointer",opacity:0.5,display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:10,background:T.surface,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:T.muted,flexShrink:0}}>{m.name.split(" ").map(w=>w[0]).slice(0,2).join("")}</div><div><div style={{fontSize:14,fontWeight:600}}>{m.name}</div><div style={{fontSize:12,color:T.muted}}>{m.classification}</div></div></div>))}
          </>}
        </>}
      </div>
    </div>
...
const PTABS=[
  {id:"reports",icon:"📋",label:"Reports"},{id:"time",icon:"⏱️",label:"Time"},
  {id:"crew",icon:"🚜",label:"Crew"},{id:"subs",icon:"🏢",label:"Subs"},
  {id:"safety",icon:"⛑️",label:"Safety"},{id:"qc",icon:"✅",label:"QC"},
  {id:"docs",icon:"📁",label:"Docs"},{id:"schedule",icon:"📅",label:"Schedule"},
  {id:"photos",icon:"📷",label:"Photos"},{id:"weather",icon:"🌤️",label:"Weather"},
...
function ProjectDetail({project:initP,user,onBack,onProjectUpdated}){
  ...
  async function load(silent=false){
    ...
    try{const [reps,saf,phs,wx]=await Promise.all([API.reports.forProject(project.id),API.safety.forProject(project.id),API.photos.forProject(project.id),API.weather.forProject(project.id)]);setReports(reps||[]);setSafety(saf||[]);setPhotos(phs||[]);setWeather(wx||[]);}
    catch(e){setErr(e.message);}if(!silent)setLoading(false);
  }
  ...
  async function flagReport(id,pm_notes){
    try{
      ...
      await notify("report_flagged","Report Flagged",`A report for ${project.name} was flagged: ${pm_notes}`,{project_id:project.id,report_id:id});
      await load(true);
  ...
  async function archiveProject(){if(!window.confirm(project.status==="active"?"Archive this job?":"Restore this job?"))return;await updateProject({status:project.status==="active"?"archived":"active"});onBack();}
    ...
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:"inherit"}}>
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"14px 16px",position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
          <div style={{flex:1,minWidth:0,paddingRight:10}}><div style={{fontSize:19,fontWeight:900,color:T.text,letterSpacing:"-0.4px",lineHeight:1.2}}>{project.name}</div><div style={{fontSize:12,color:T.sub,marginTop:2}}>{[project.client,project.location].filter(Boolean).join(" · ")||"No details"}</div></div>
          <span style={pill(project.status==="active"?T.green:T.muted)}>{project.status}</span>
        </div>
        <StatBar items={[{label:"Reports",val:reports.length,color:T.orange},{label:"Labor",val:"$"+(tot.l>=1000?(tot.l/1000).toFixed(1)+"k":fmt(tot.l)),color:T.green},{label:"Equip",val:"$"+(tot.e>=1000?(tot.e/1000).toFixed(1)+"k":fmt(tot.e)),color:T.yellow},{label:"Total",val:"$"+(tot.g>=1000?(tot.g/1000).toFixed(1)+"k":fmt(tot.g)),color:T.blue}]}/>
      ...
      <div style={{padding:"14px 16px 80px"}}>
        ...
        {!loading&&tab==="qc"      &&<QCTab projectId={project.id} user={user} onErr={setErr}/>}
        {!loading&&tab==="docs"    &&<DocsTab projectId={project.id} user={user} onErr={setErr}/>}
        {!loading&&tab==="schedule"&&<ScheduleTab projectId={project.id} user={user} onErr={setErr}/>}
...
function CustomReports({projects,reports}){
  ...
  const filteredReports=reports.filter(r=>{const start=getStart();const matchDate=r.date>=start;const matchProj=selProject==="all"||r.project_id===selProject;return matchDate&&matchProj;});

  const totals=filteredReports.reduce((s,r)=>{const t=reportTotals(r);return{labor:s.labor+t.labor,equip:s.equip+t.equip,mats:s.mats+t.mats,grand:s.grand+t.grand};},{labor:0,equip:0,mats:0,grand:0});

  const workerMap={};
  filteredReports.forEach(r=>(r.labor||[]).forEach(l=>{if(!l.name)return;if(!workerMap[l.name])workerMap[l.name]={name:l.name,reg:0,ot:0,travel:0,pay:0};workerMap[l.name].reg+=parseFloat(l.regHrs)||0;workerMap[l.name].ot+=parseFloat(l.otHrs)||0;workerMap[l.name].travel+=parseFloat(l.travelHrs)||0;workerMap[l.name].pay+=laborAmt(l);}));
  const workerRows=Object.values(workerMap).sort((a,b)=>b.pay-a.pay);

  const projectMap={};
  projects.forEach(p=>{projectMap[p.id]={name:p.name,labor:0,equip:0,mats:0,grand:0,count:0};});
  filteredReports.forEach(r=>{if(!projectMap[r.project_id])return;const t=reportTotals(r);projectMap[r.project_id].labor+=t.labor;projectMap[r.project_id].equip+=t.equip;projectMap[r.project_id].mats+=t.mats;projectMap[r.project_id].grand+=t.grand;projectMap[r.project_id].count++;});
  const projRows=Object.values(projectMap).filter(p=>p.count>0).sort((a,b)=>b.grand-a.grand);

  function exportReport(){
    const wb=XLSX.utils.book_new();
    // Summary sheet
    const sumRows=[
      ["AIME Field OS — Custom Report"],
      [`Period: ${range.toUpperCase()} · Project: ${selProject==="all"?"All":projects.find(p=>p.id===selProject)?.name||"—"}`],
      [`Generated: ${new Date().toLocaleString()}`],[],
      ["TOTALS"],["Labor","Equipment","Materials","Grand Total"],
      [totals.labor,totals.equip,totals.mats,totals.grand],[],
      ["WORKERS"],["Name","Reg Hrs","OT Hrs","Travel Hrs","Total Pay"],
      ...workerRows.map(w=>[w.name,w.reg,w.ot,w.travel,w.pay]),
      [],["PROJECTS"],["Project","Reports","Labor","Equipment","Materials","Total"],
      ...projRows.map(p=>[p.name,p.count,p.labor,p.equip,p.mats,p.grand]),
    ];
    const ws1=XLSX.utils.aoa_to_sheet(sumRows);
    ws1["!cols"]=[{wch:30},{wch:15},{wch:15},{wch:15},{wch:15},{wch:15}];
    XLSX.utils.book_append_sheet(wb,ws1,"Summary");
    // Detail sheet
    const detailRows=[["Date","Project","Submitted By","Status","Labor","Equipment","Materials","Grand Total"]];
    filteredReports.forEach(r=>{const t=reportTotals(r);const p=projects.find(x=>x.id===r.project_id);detailRows.push([r.date,p?.name||"Unknown",r.submitted_by||"",r.status||"",t.labor,t.equip,t.mats,t.grand]);});
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
      <div><label style={lbl}>Time Range</label><select value={range} onChange={e=>setRange(e.target.value)} style={inp}><option value="week">This Week</option><option value="month">This Month</option><option value="quarter">This Quarter</option><option value="year">This Year</option><option value="all">All Time</option></select></div>
      <div><label style={lbl}>Project</label><select value={selProject} onChange={e=>setSelProject(e.target.value)} style={inp}><option value="all">All Projects</option>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
    </div>
    <div style={{...cardS,marginBottom:14,background:T.orangeLow,border:`1px solid ${T.orange}40`}}>
      <div style={{fontSize:11,color:T.orange,fontWeight:700,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>Totals — {filteredReports.length} Reports</div>
      {[["Labor",totals.labor,T.green],["Equipment",totals.equip,T.yellow],["Materials",totals.mats,T.blue],["Grand Total",totals.grand,T.orange]].map(([l,v,c])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:13,color:T.sub,fontWeight:l==="Grand Total"?700:400}}>{l}</span><span style={{fontSize:l==="Grand Total"?18:13,fontWeight:800,color:c}}>${fmt(v)}</span></div>))}
    </div>
    {projRows.length>0&&<div style={{...cardS,marginBottom:14}}><div style={{fontSize:12,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>By Project</div>{projRows.map(p=>(<div key={p.name} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${T.border}`}}><div><div style={{fontSize:13,fontWeight:700}}>{p.name}</div><div style={{fontSize:11,color:T.muted}}>{p.count} reports</div></div><div style={{fontSize:15,fontWeight:800,color:T.green}}>${fmt(p.grand)}</div></div>))}</div>}
    {workerRows.length>0&&<div style={{...cardS,marginBottom:14}}><div style={{fontSize:12,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>By Worker</div>{workerRows.map(w=>(<div key={w.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${T.border}`}}><div><div style={{fontSize:13,fontWeight:700}}>{w.name}</div><div style={{fontSize:11,color:T.muted}}>{w.reg.toFixed(1)}reg {w.ot.toFixed(1)}OT {w.travel.toFixed(1)}tr hrs</div></div><div style={{fontSize:14,fontWeight:800,color:T.green}}>${fmt(w.pay)}</div></div>))}</div>}
...
function NotificationsPanel({onClose}){
  ...
  async function load(){setLoading(true);try{setNotifs(await API.notifications.list()||[]);}catch{}setLoading(false);}
  useEffect(()=>{load();},[]);
  async function markAllRead(){try{await API.notifications.markAllRead();await load();}catch{}}
  const unread=notifs.filter(n=>!n.read).length;
  const typeIcon={report_submitted:"📋",report_flagged:"🚩",report_approved:"✅"};
  return(
    <div style={{background:T.bg,minHeight:"100vh",fontFamily:"inherit"}}>
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"14px 16px",position:"sticky",top:0,zIndex:50}}>
        <button onClick={onClose} style={{background:"none",border:"none",color:T.sub,fontSize:13,cursor:"pointer",marginBottom:8,padding:0,fontFamily:"inherit"}}>← Back</button>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{fontSize:20,fontWeight:900}}>🔔 Notifications{unread>0&&<span style={{marginLeft:8,background:T.orange,color:"#09090B",borderRadius:20,padding:"2px 8px",fontSize:12,fontWeight:800}}>{unread}</span>}</div>
          {unread>0&&<button onClick={markAllRead} style={{background:"none",border:"none",color:T.muted,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>Mark all read</button>}
        </div>
      </div>
      <div style={{padding:"14px 16px 80px"}}>
        {loading&&<Spinner/>}
        {!loading&&notifs.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:T.muted}}><div style={{fontSize:36,marginBottom:8}}>🔔</div><div>No notifications yet.</div></div>}
        {!loading&&notifs.map(n=>(<div key={n.id} onClick={async()=>{if(!n.read){await API.notifications.markRead(n.id);await load();}}} style={{...cardS,marginBottom:8,borderLeft:`3px solid ${n.read?T.border:T.orange}`,opacity:n.read?0.6:1,cursor:n.read?"default":"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div style={{display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:18,flexShrink:0}}>{typeIcon[n.type]||"📬"}</span><div><div style={{fontSize:14,fontWeight:700}}>{n.title}</div>{n.body&&<div style={{fontSize:12,color:T.sub,marginTop:2,lineHeight:1.4}}>{n.body}</div>}<div style={{fontSize:11,color:T.muted,marginTop:4}}>{n.created_at?new Date(n.created_at).toLocaleString():""}</div></div></div>{!n.read&&<div style={{width:8,height:8,borderRadius:"50%",background:T.orange,flexShrink:0,marginTop:4}}/>}</div></div>))}
      </div>
    </div>
  );
}
...
function NotifSettings({user}){
  const [settings,setSettings]=useState({pm_email:"",notify_on_submit:true,notify_on_flag:true});
  const [saving,setSaving]=useState(false); const [saved,setSaved]=useState(false);
  useEffect(()=>{API.notifSettings.get(user.name).then(rows=>{if(rows?.length>0)setSettings(rows[0]);}).catch(()=>{});},[]);
  async function save(){setSaving(true);try{await API.notifSettings.upsert({pm_name:user.name,...settings});setSaved(true);setTimeout(()=>setSaved(false),2000);}catch{}setSaving(false);}
  return(
    <div style={{...cardS,marginTop:14}}>
      <div style={{fontSize:12,fontWeight:700,color:T.orange,textTransform:"uppercase",letterSpacing:"1px",marginBottom:14}}>🔔 Notification Settings</div>
      <div style={{marginBottom:12}}><label style={lbl}>Your Email (for future integrations)</label><input type="email" placeholder="yourname@company.com" value={settings.pm_email||""} onChange={e=>setSettings(s=>({...s,pm_email:e.target.value}))} style={inp}/></div>
      <div style={{marginBottom:12}}>
        {[["notify_on_submit","Notify when report submitted"],["notify_on_flag","Notify when report flagged"]].map(([k,l])=>(<div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:14}}>{l}</span><button onClick={()=>setSettings(s=>({...s,[k]:!s[k]}))} style={{width:44,height:24,borderRadius:12,background:settings[k]?T.orange:T.border,border:"none",cursor:"pointer",position:"relative",transition:"background 0.2s"}}><div style={{position:"absolute",top:3,left:settings[k]?22:3,width:18,height:18,borderRadius:"50%",background:"#fff",transition:"left 0.2s"}}/></button></div>))}
      </div>
      <div style={{...cardS,background:T.surface,marginBottom:12}}><div style={{fontSize:12,color:T.muted,lineHeight:1.6}}>In-app notifications are live now. For email alerts, connect Supabase Edge Functions with Resend (resend.com). Instructions at docs.aime-field-os.com</div></div>
      <button onClick={save} style={{...primBtn,borderRadius:12,background:saved?T.green:T.orange,color:"#09090B"}}>{saving?"Saving…":saved?"✓ Saved!":"Save Settings"}</button>
    </div>
  );
}
...
function PMDashboard({onBack,user}){
  ...
  const [unread,setUnread]=useState(0); const [showNotifs,setShowNotifs]=useState(false);
  ...
  if(showNotifs) return <NotificationsPanel onClose={()=>{setShowNotifs(false);load();}}/>;
  ...
  const DMTABS=[{id:"overview",l:"📊 Overview"},{id:"approvals",l:`✅ Approvals${pending.length>0?" ("+pending.length+")":""}`},{id:"workers",l:"👷 Workers"},{id:"billing",l:"💰 Billing"},{id:"reports",l:"📄 Reports"},{id:"settings",l:"⚙️ Settings"}];
          ...
          <>
            {pmTab==="overview"&&(<div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
                {[{icon:"💰",label:"Total Billed",val:"$"+fmt(allTot.g),color:T.green},{icon:"📋",label:"Reports This Week",val:thisWeek.length,color:T.orange},{icon:"🏗️",label:"Active Jobs",val:projects.filter(p=>p.status==="active").length,color:T.blue},{icon:"✅",label:"Pending Approvals",val:pending.length,color:pending.length>0?T.yellow:T.muted}].map(k=>(<div key={k.label} style={cardS}><div style={{fontSize:24,marginBottom:6}}>{k.icon}</div><div style={{fontSize:22,fontWeight:900,color:k.color,letterSpacing:"-0.5px"}}>{k.val}</div><div style={{fontSize:11,color:T.muted,marginTop:3}}>{k.label}</div></div>))}
              </div>
            ...
            {pmTab==="approvals"&&(<div>
              ...
              {pending.map(r=>{const t=reportTotals(r);const p=projects.find(x=>x.id===r.project_id);return(<div key={r.id} style={{...cardS,marginBottom:12,borderLeft:`3px solid ${T.yellow}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}><div><div style={{fontSize:15,fontWeight:800}}>{fmtShort(r.date)}</div><div style={{fontSize:12,color:T.sub}}>{r.projects?.name||"Unknown Job"}</div><div style={{fontSize:12,color:T.muted}}>by {r.submitted_by||"Unknown"}</div>{r.description&&<div style={{fontSize:12,color:T.sub,marginTop:4,lineHeight:1.4}}>{r.description.slice(0,100)}{r.description.length>100?"…":""}</div>}</div><div style={{textAlign:"right",flexShrink:0,marginLeft:10}}><div style={{fontSize:18,fontWeight:900,color:T.green}}>${fmt(t.grand)}</div><div style={{display:"flex",gap:4,marginTop:6}}>{(r.labor||[]).length>0&&<span style={pill(T.orange)}>👷{r.labor.length}</span>}{(r.equipment||[]).length>0&&<span style={pill(T.yellow)}>🚜{r.equipment.length}</span>}</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}><button onClick={()=>approve(r.id)} style={{...primBtn,background:T.greenLow,color:T.green,border:`1px solid ${T.green}40`,borderRadius:10,padding:"12px"}}>✓ Approve</button><button onClick={()=>{const n=window.prompt("Flag note for crew:");if(n!==null)flag(r.id,n);}} style={{...primBtn,background:T.redLow,color:T.red,border:`1px solid ${T.red}40`,borderRadius:10,padding:"12px"}}>🚩 Flag</button></div><button onClick={()=>{setActiveReport(r);setActiveProject(p||{id:r.project_id,name:r.projects?.name||"Unknown",...(p||{})});}} style={{...ghostBtn,width:"100%",textAlign:"center",padding:"10px"}}>View Full Report →</button></div>);})}
            </div>)}
            {pmTab==="workers"&&(<div>
            ...
            {pmTab==="billing"&&(<div>
              <div style={{...cardS,marginBottom:16,background:T.orangeLow,border:`1px solid ${T.orange}40`}}>
                ...
                {[["Labor",allTot.l,T.green],["Equipment",allTot.e,T.yellow],["Materials",allTot.m,T.blue],["Grand Total",allTot.g,T.orange]].map(([l,v,c])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:14,color:T.sub,fontWeight:l==="Grand Total"?800:400}}>{l}</span><span style={{fontSize:l==="Grand Total"?20:14,fontWeight:800,color:c}}>${fmt(v)}</span></div>))}
              </div>
              <div style={{fontSize:12,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:12}}>By Job</div>
              {projRows.map(p=>(<div key={p.id} style={{...cardS,marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}><div><div style={{fontSize:15,fontWeight:700}}>{p.name}</div><div style={{fontSize:11,color:T.muted}}>{p.count} reports · {p.location||"No location"}</div></div><div style={{fontSize:18,fontWeight:900,color:T.green}}>${p.grand>=1000?(p.grand/1000).toFixed(1)+"k":fmt(p.grand)}</div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>{[["Labor",p.labor,T.orange],["Equip",p.equip,T.yellow],["Mats",p.mats,T.blue]].map(([l,v,c])=>(<div key={l} style={{background:T.surface,borderRadius:8,padding:"8px",textAlign:"center"}}><div style={{fontSize:13,fontWeight:700,color:c}}>${v>=1000?(v/1000).toFixed(1)+"k":fmt(v)}</div><div style={{fontSize:9,color:T.muted,textTransform:"uppercase",letterSpacing:"0.5px"}}>{l}</div></div>))}</div></div>))}
            </div>)}
            {pmTab==="reports"&&<CustomReports projects={projects} reports={reports}/>}
            {pmTab==="settings"&&<NotifSettings user={user}/>}
          </>
        )}
      </div>
    </div>
...
export default function App(){
  ...
  async function loadProjects(){
    ...
    try{
      ...
      const enriched=await Promise.all((projs||[]).map(async p=>{
        try{const reps=await API.reports.forProject(p.id);const billed=(reps||[]).reduce((s,r)=>s+reportTotals(r).grand,0);const sorted=[...(reps||[])].sort((a,b)=>b.date>a.date?1:-1);return{...p,_reports:(reps||[]).length,_billed:billed,_lastReport:sorted[0]?.date||null};}
  ...
  if(!user) return(<div style={{maxWidth:480,margin:"0 auto",fontFamily:"'DM Sans',system-ui,sans-serif"}}><LoginScreen onLogin={u=>setUser(u)}/></div>);

  return(
    <div style={{maxWidth:480,margin:"0 auto",fontFamily:"'DM Sans',system-ui,sans-serif",background:T.bg,minHeight:"100vh"}}>
      {err&&<div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,zIndex:200,padding:"0 16px"}}><ErrBanner msg={err} onDismiss={()=>setErr("")}/></div>}
      ...
      {screen==="crewDirectory"&&<CrewDirectoryScreen user={user} onBack={()=>setScreen("projects")}/>}
      {screen==="projectDetail"&&activeProject&&(
        <ProjectDetail
          ...
          onProjectUpdated={u=>{setActiveProject(p=>({...p,...u}));loadProjects();}}
        />
      )}
      {screen==="projects"&&(
        <ProjectsHome
